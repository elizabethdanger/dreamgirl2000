...
function getActiveSpans(el) {
	...
	if (word) {
		wordSpans = [
			...document.querySelectorAll(`[data-word="${word}"`)
		];
	} else if (special) {
		wordSpans = [
...
Game.prototype.handleSpecial = function (target) {
	...
	if (Math.random() < 0.66 || this.lives === MAX_LIVES) {
		...
		let dudSpans = [
			...document.querySelectorAll(`[data-word="${dudNr}"]`)
		];
...
Game.prototype.error = async function (pw) {
	...
	let likeness = pw
		...
		.reduce(
			(total, c, i) =>
				(total += Number(c === this.password[i])),
			0
		);
...
Game.prototype.start = async function () {
	await type(
		[
			"Welcome to DREAMGIRL2000(TM) admin console.",
			">RUN PASSWORD_CHOOM.ICE"
		],
		FAST,
		this.container
	);
